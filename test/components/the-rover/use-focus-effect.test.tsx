...
import useFocus from "../../../src/components/the-rover/use-focus-effect";
...
describe("useFocus", () => {
	test("does not focus on mount when false", () => {
		...
		const { rerender } = renderHook(() => useFocus(false, mockRef));
	...
	test("focuses on mount when true", () => {
		...
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		const mockRef = { current: { focus: focusMock } } as React.RefObject<any>;
		const { rerender } = renderHook(() => useFocus(true, mockRef));
	...
	test("focuses when focus value changes to true", () => {
		...
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		const mockRef = { current: { focus: focusMock } } as React.RefObject<any>;
		const { rerender } = renderHook(() => useFocus(focused, mockRef));
